...
function Register({ navigation }) {
  ...
  const registerRequest = values => {
    ...
    apiCustomer
      .post('/auth/register', { person_type: 'individual', user: { ...values },user_type: "client" })
      .then(resp => {
        dispatch(UserActions.loginRequest(values.email, values.password));
        const { token } = resp.data;
        dispatch(UserActions.setToken(token));
        navigation.navigate('REGISTER_OWNER_ADDRESS');
      })
      .catch(err => {
              ...
              <ContainerInput style={{ paddingRight: 20 }}>
                ...
                <View style={{ paddingRight: 20 }}>
                  <TextSmall color="#F6BE67" bold>
                    Após avançar você vai precisar confirmar sua conta.
                  </TextSmall>
                </View>
              </ContainerInput>
              <ButtonContainer>
